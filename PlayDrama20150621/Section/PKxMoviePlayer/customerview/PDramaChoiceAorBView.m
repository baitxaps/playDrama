...
@interface PDramaChoiceAorBView()
...
@implementation PDramaChoiceAorBView


+ (PDramaChoiceAorBView *) initWithNib
{
    PDramaChoiceAorBView *view = nil;
    NSArray *array = [[NSBundle mainBundle] loadNibNamed:@"PDramaChoiceAorBView" owner:self options:nil];
    for (id obj in array) {
        if ([obj isKindOfClass:[PDramaChoiceAorBView class]]) {
            view = (PDramaChoiceAorBView *)obj;
            break;
        }
    }
    return view;
...
- (void) awakeFromNib
{
    ...
    UITapGestureRecognizer *gs = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(hide)];
