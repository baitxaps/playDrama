...
@interface PMoviePlayerView()
{
    PChooseDramaView    *chooseDramaView;
    PDramaChoiceAorBView*choiceAorBView;
...
@property (assign,nonatomic) BOOL                           isRotate ,showChooseView;
...
@implementation PMoviePlayerView
...
{
    PMoviePlayerView *view = [[NSBundle mainBundle]loadNibNamed:@"PMoviePlayerView"
                                                          owner:self
                                                        options:nil][0];
    return view;
}
                 ...
                 parameters: (NSDictionary *) parameters
{
    ...
    decoder.interruptCallback = ^BOOL(){
        __strong PMoviePlayerView *strongSelf = weakSelf;
...
- (void) updatePosition: (CGFloat) position playMode: (BOOL) playMode
{
    ...
    __weak PMoviePlayerView *weakSelf = self;
...
{
    [self handleTap:nil];
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"失败", nil)
...
- (IBAction)dramaHandle:(id)sender
{
    ...
    //2.加载选集View
    if (choiceAorBView == nil) {
        choiceAorBView = [PDramaChoiceAorBView initWithNib];
        choiceAorBView.dataArray = [@[@"http://static.tripbe.com/videofiles/20121214/9533522808.f4v.mp4",
                                      @"http://www.qeebu.com/newe/Public/Attachment/99/52958fdb45565.mp4"]mutableCopy];
        CGRect  frame  = choiceAorBView.frame;
        frame.origin.y = UIScreenHeight;
        choiceAorBView.frame = frame;
        [self addSubview:choiceAorBView];
    }
    CGRect  frame    = choiceAorBView.frame ;
    CGFloat bottomH  = CGRectGetHeight(_bottomContainerView.frame);
    CGFloat choiceH  = CGRectGetHeight(choiceAorBView.frame);
    ...
    WS(weakSelf);
    choiceAorBView.dramaOptionBlock = ^(NSString *urlString){
        [weakSelf hideChoiceAorBView];
        [weakSelf state];
        [weakSelf remove];
        [weakSelf viewToPrepare];
        [weakSelf playWithContentPath:urlString parameters:nil];
    };
}

#pragma mark- 隐藏剧集容器

- (void)hideChoiceAorBView
{    
    [choiceAorBView hide];
