...
static int readFromNSInputStreamFunction(void* opaque, uint8_t* buf, int buf_size)
{
    __unsafe_unretained NSInputStream *stream = (__bridge NSInputStream *)opaque;
    
    int numBytes = [stream read: (char*)buf maxLength: buf_size];
...
- (kxMovieError) openInput: (NSString *) path
{
    ...
    //    formatCtx->pb = avio_alloc_context(ioBuffer, ioBufferSize, 0, (__bridge void)(stream), &readFromNSInputStreamFunction, NULL, NULL);
    //    formatCtx->pb->seekable = 0;
    //    formatCtx->pb->write_flag = 0;
    
    //avformat_open_input会自动为空AVFormatContext分配内存并初始化
...
- (kxMovieError) openVideoStream
{
    ...
    _videoStream = -1;
    _artworkStream = -1;
    //判断formatcontext中视频和音频流等的位置
    _videoStreams = collectStreams(_formatCtx, AVMEDIA_TYPE_VIDEO);
    for (NSNumber *n in _videoStreams) {
        
        const NSUInteger iStream = n.integerValue;
        //判断有没有其他流附着在视频流中，一般为艺术家信息
...
- (kxMovieError) openVideoStream: (NSInteger) videoStream
{
    ...
    AVStream *st = _formatCtx->streams[_videoStream];
    avStreamFPSTimeBase(st, 0.04, &_fps, &_videoTimeBase);//设置视频的帧率、时间基准等
...
- (kxMovieError) openAudioStream: (NSInteger) audioStream
{
    ...
    if (!audioCodecIsSupported(codecCtx)) {
        //如果音频格式不是AV_SAMPLE_FMT_S16，需要转AV_SAMPLE_FMT_S16
        id<KxAudioManager> audioManager = [KxAudioManager audioManager];
        //swr_alloc_set_opts: handles audio resampling, sample format conversion and mixing
