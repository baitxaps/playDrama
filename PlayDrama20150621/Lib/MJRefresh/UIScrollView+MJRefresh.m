...
- (void)addFooterWithCallback:(void (^)())callback
{
    ...
    if (!self.footer) {
        ...
        footer.pullingText = self.footerPullingText;
     ...
     */
    [self addFooterWithText:nil callback:callback];
}

- (void)addFooterWithText:(NSString *)text callback:(void (^)())callback
{
    // 1.创建新的footer
    if (!self.footer) {
        MJRefreshFooterView *footer = [MJRefreshFooterView footer];
        ...
        self.footer = footer;
        if (text != nil) {
            footer.pullingText = text;
            footer.state = 0;
            [footer setState:MJRefreshStateNormal];
        }
    }
